...
#include <cstdint>
  ...
  struct Environment : public std::enable_shared_from_this<Environment>
  {
    ...
    // Cache for quick lookup of variables in this environment and its ancestors
    std::unordered_map<std::string, std::pair<std::shared_ptr<Environment>, size_t>> lookup_cache;
    bool cache_valid = true; // Flag to indicate if cache needs rebuilding
    ...
    // Rebuild cache for efficient lookups
    void rebuildCache();
  };
