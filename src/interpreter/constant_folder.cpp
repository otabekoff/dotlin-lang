...
Statement::Ptr ConstantFolderVisitor::fold(Statement::Ptr stmt) {
  if (!stmt)
    return nullptr;
  optResultStmt = nullptr;
  stmt->accept(*this);
  if (optResultStmt) {
    Statement::Ptr res = optResultStmt; // Use assignment for shared_ptr
    optResultStmt = nullptr;
    return res;
  }
  return stmt;
...
void ConstantFolderVisitor::visit(IfStmt &node) {
  ...
  if (cLit && std::holds_alternative<bool>(cLit->value)) {
    if (std::get<bool>(cLit->value)) {
      optResultStmt = node.thenBranch;
    } else {
      if (node.elseBranch.has_value())
        optResultStmt = node.elseBranch.value();
      else
        optResultStmt = std::make_shared<BlockStmt>(
            std::vector<Statement::Ptr>(), node.line, node.column);
    }
    return;
  }
  optResultStmt = nullptr;
...
void ConstantFolderVisitor::visit(WhileStmt &node) {
  ...
  if (cLit && std::holds_alternative<bool>(cLit->value) &&
    ...
    optResultStmt = std::make_shared<BlockStmt>(std::vector<Statement::Ptr>(),
                                                node.line, node.column);
    return;
  }
  optResultStmt = nullptr;
