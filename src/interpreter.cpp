...
namespace dotlin
{
  ...
  // Static map to store function definitions - now maps function name to vector of overloads
  static std::map<std::string, std::vector<std::shared_ptr<FunctionDef>>> functionDefinitions;
  ...
  void Interpreter::visit(FunctionDeclStmt &node)
  {
    ...
    auto funcDef = std::make_shared<FunctionDef>(node.name, std::move(paramsCopy), std::move(node.body));
  ...
  {
    // Register the class in the environment for later instantiation
    // For now, we'll just store a marker to indicate the class exists
    std::string className = "class:" + node.name;
    environment->define(node.name, Value(className));

    // In a full implementation, we would store the class definition
    // including its properties, methods, and inheritance information
    // For now, we're just acknowledging the class declaration
  }

  void Interpreter::visit(ConstructorDeclStmt &node)
  {
    // Register the constructor in the environment for the corresponding class
    // For now, we'll just acknowledge the constructor declaration
    // In a full implementation, we would store the constructor definition
    // along with its parameters and body for later instantiation
  }

  void Interpreter::visit(LambdaExpr &node)
  {
    // For now, we'll just acknowledge the lambda expression
    // The actual lambda value creation and handling happens in the expression evaluation visitor
    // This method is for statement execution context, not expression evaluation
  }
  ...
  Value Interpreter::evaluate(Expression &expr)
  {
    ...
    struct EvalVisitor : public AstVisitor
    {
      ...
      void visit(LambdaExpr &node) override
      {
        ...
        // Since we can't copy unique_ptr from the original AST, we'll create a new LambdaValue
        // with copied parameters and a nullptr for body (we'll handle execution differently)
        std::vector<FunctionParameter> paramsCopy;
        for (const auto &param : node.parameters)
        {
        ...
        // We can't copy the body (unique_ptr), so we'll need to handle lambda execution
        // by referencing the original AST node when the lambda is called
        // For now, pass a nullptr as body, and store a reference to the original node
        auto lambdaValue = std::make_shared<LambdaValue>(std::move(paramsCopy), nullptr, interpreter->environment, &node);
        result = lambdaValue;
      }
      ...
      void visit(CallExpr &node) override
      {
        ...
        // First, try to evaluate the callee to handle complex expressions (like lambda calls)
        ...
        if (std::holds_alternative<std::shared_ptr<LambdaValue>>(calleeValue))
        {
          ...
          for (size_t i = 0; i < lambda->parameters.size() && i < args.size(); ++i)
          {
          ...
          try
          {
            if (lambda->body)
            {
              if (auto *blockBody = dynamic_cast<BlockStmt *>(lambda->body.get()))
              {
                for (const auto &stmt : blockBody->statements)
                {
                  if (stmt)
            ...
            // If lambda body is null but original_node exists, execute the original lambda body
            else if (lambda->original_node && lambda->original_node->body)
            {
              if (auto *blockBody = dynamic_cast<BlockStmt *>(lambda->original_node->body.get()))
              {
                // For block statements, execute all but the last as statements and evaluate the last as expression
                if (!blockBody->statements.empty())
                {
                  ...
                  // Evaluate the last statement as an expression and return its value
                  ...
                  if (lastStmt)
                  {
                    ...
                    {
                      // For other types of statements, execute them but this may not return a value
              ...
              {
                // If the body is a single expression (not in a block), evaluate it as an expression
                // First, check if it's an expression statement
                if (auto *exprStmt = dynamic_cast<ExpressionStmt *>(lambda->original_node->body.get()))
                {
                  returnValue = interpreter->evaluate(*exprStmt->expression);
                }
                else
          ...
          catch (const std::runtime_error &e)
          {
            ...
            if (msg.substr(0, 7) == "RETURN:")
            {
              ...
              // Check if the string contains a decimal point to determine if it's an int or double
        ...
        if (identifier)
        {
          auto selectedFunc = interpreter->findBestFunctionOverload(identifier->name, args);
          ...
          if (selectedFunc)
          {
            ...
            // Bind parameters to arguments
            for (size_t i = 0; i < selectedFunc->parameters.size() && i < args.size(); ++i)
            {
            ...
            try
            {
              ...
              {
                if (auto *blockBody = dynamic_cast<BlockStmt *>(selectedFunc->body.get()))
                {
                  for (const auto &stmt : blockBody->statements)
                  {
                    if (stmt)
          ...
          std::cout << "Error: No matching function overload found for " << identifier->name << " with " << args.size() << " arguments" << std::endl;
  ...
  Interpreter::execute(Statement &stmt)
  {
    ...
    struct ExecVisitor : public AstVisitor
    {
      ...
      void visit(ConstructorDeclStmt &node) override { (void)node; /* For now, just acknowledge the constructor */ }
  ...
  public:
    ...
    std::shared_ptr<dotlin::Type> checkExpression(dotlin::Expression &expr)
    {
      ...
      struct TypeCheckVisitor : public dotlin::AstVisitor
      {
        ...
        void visit(dotlin::LambdaExpr &node) override { result = std::make_shared<dotlin::Type>(dotlin::TypeKind::UNKNOWN); }
        void visit(dotlin::ClassDeclStmt &node) override { result = std::make_shared<dotlin::Type>(dotlin::TypeKind::VOID); }
        void visit(dotlin::ConstructorDeclStmt &node) override { result = std::make_shared<dotlin::Type>(dotlin::TypeKind::VOID); }
      };
  ...
  std::shared_ptr<FunctionDef> Interpreter::findBestFunctionOverload(const std::string &name, const std::vector<Value> &args)
  {
